...
_section_word_re = re.compile(r'^[A-Za-z0-9_-]+$')
        ...
        if line[0] == '[':
            ...
            if line[-1] != "}":
                raise TomlDecodeError("Line breaks are not allowed in inline"
                                      "objects")
            _load_inline_object(line, currentlevel, multikey,
                                multibackslash)
        elif "=" in line:
            ret = _load_line(line, currentlevel, multikey,
                             multibackslash)
            if ret is not None:
...
def _load_inline_object(line, currentlevel, multikey=False,
                        multibackslash=False):
    ...
    for group in groups:
        status = _load_line(group, currentlevel, multikey,
                            multibackslash)
...
def _load_date(val):
    ...
    try:
        ...
        tz = None
    try:
        return '%02d-%02d-%02dT%02d:%02d:%02d.%d%s' % (
            int(val[:4]), int(val[5:7]),
            int(val[8:10]), int(val[11:13]),
            int(val[14:16]), int(val[17:19]),
            microsecond,
            tz,
        )
    except ValueError:
...
def _dump_sections(o, sup, preserve=False):
    ...
    for section in o:
        qsection = section
        if not _section_word_re.match(section):
            if '"' in section:
                qsection = "'" + section + "'"
            else:
                qsection = '"' + section + '"'
        ...
        elif preserve and isinstance(o[section], dict) and o[section].pop('@@@inline', False):
